...
const Profile = () => {
  ...
  const uplaodProfile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      setLoading(true)
      ...
      const uploading = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/student/dashboard/uploadFile`,
        formData,
        {
          headers: {
      ...
      // console.log('uploading: ', uploading.data.data);
      if (uploading?.data?.status) {
        setLoading(false)
        dispatch(getStudentData(uploading.data.data))
      } else {
        alert("image not uplaoded")
      }
      setResponse(uploading.data);
    } catch (error) {
      setLoading(false)
  ...
  const uplaodCoverProfile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    // console.log("upload cover profile")
    try {
      ...
      // Send request to the API
      const uploading = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/student/dashboard/change/cover/pic`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <DefaultLayout>
      <div className="mx-auto max-w-242.5">
        ...
        <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          // style={{ backgroundImage: `url('${data.cover ? data.cover : '/images/cover/cover-01.png'}')`, backgroundRepeat: "no-repeat", backgroundSize: "contain" }}
          >
            <Image
              src={data.cover ? data.cover : "/images/cover/cover-01.png"}
              alt="profile cover"
            ...
            <div className="absolute bottom-1 right-1 z-10 xsm:bottom-4 xsm:right-4">
              ...
              >
                {coverLoading ? <FontAwesomeIcon className="loadingButton" icon={faFan} /> : <>
                  <input
                    type="file"
                    name="cover"
                    id="cover"
                    className="sr-only"
                    onChange={(e) => uplaodCoverProfile(e)}
                  />
          ...
          <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
            <div className="relative z-30 mx-auto -mt-12 h-30 w-full max-w-30 rounded-full bg-white/20 backdrop-blur sm:h-44 sm:mb-10 sm:max-w-44">
              <div className="relative drop-shadow-2 h-full w-full ">
                <Image
                  src={data.profile ? data.profile : "/images/user/default-user-image.png"}
                  width={500}
                  height={500}
                  priority={true}
                  style={{
                ...
                >
                  {loading ? <FontAwesomeIcon className="loadingButton" icon={faFan} /> :
                    <>
                      <svg
                        className="fill-current"
                        width="14"
